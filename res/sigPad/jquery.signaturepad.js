...
/* Modified by Yao Zhao
 * Feb 15, 2013
 * Added compression funtions from 
 * https://gist.github.com/jaboc83/2559996
 * please see sigPadCompression.js
 **/
...
function SignaturePad (selector, options) {
  ...
  function stopDrawing () {
    ...
    if (settings.output && output.length > 0)
        {   
            var sigCompressed = deflateFromJsonSignature(JSON.stringify(output))
            /*$(settings.output, context).val(JSON.stringify(output)) //YAO*/
            $(settings.output, context).val(sigCompressed)
        }
  }
  ...
  $.extend(self, {
    ...
    , regenerate : function (paths) {
      ...
      if (typeof paths === 'string'){
          paths = inflateToJsonSignature(paths) /* YAO */
      ...
      if (settings.output && $(settings.output, context).length > 0)
          {
              var sigCompressed = deflateFromJsonSignature(JSON.stringify(output)) /* YAO */
